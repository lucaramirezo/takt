...
- WSL2 PORT 5432 is silently dropped; takt's local Postgres is published on 15433 -> 5432 everywhere (15432 belongs to KULT's dev container; takt has its own `takt-dev-postgres-1` on 15433). Before any `db:generate`, `db:migrate`, or test run, verify DB availability with: `pg_isready -h localhost -p 15433 && echo DB_READY`. If not ready, run `pnpm up` first.
